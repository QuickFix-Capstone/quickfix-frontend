...
export default function ProviderJobDetails() {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
      <div className="rounded-xl sm:rounded-2xl border bg-white shadow-sm overflow-hidden space-y-4 sm:space-y-6">
        {/* IMAGE PLACEHOLDER */}
        <div className="relative w-full bg-neutral-100">
          <div className="aspect-[16/9] sm:aspect-[16/8] flex items-center justify-center text-neutral-400">
            <div className="flex flex-col items-center gap-2">
              ...
              <span className="text-xs sm:text-sm font-medium">Job image placeholder</span>
            </div>
          </div>
        ...
        <div className="p-4 sm:p-6 space-y-4 sm:space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 text-sm">
            <div className="rounded-lg bg-neutral-50 p-3 sm:p-4">
              <p className="font-medium text-neutral-800 text-xs sm:text-sm">Location</p>
              <p className="text-neutral-600 mt-0.5 sm:mt-1 text-sm">
                {job.location?.address
                  ? `${job.location.address}, ${job.location.city}, ${job.location.state}`
                  : `${job.location.city}, ${job.location.state}`}
              </p>
            ...
            <div className="rounded-lg bg-neutral-50 p-3 sm:p-4">
              <p className="font-medium text-neutral-800 text-xs sm:text-sm">Budget</p>
              <p className="text-green-700 font-semibold mt-0.5 sm:mt-1 text-sm">
                {job.budget?.min && job.budget?.max
                  ? `$${job.budget.min} – $${job.budget.max}`
                  : "Not specified"}
              </p>
            ...
            <div className="rounded-lg bg-neutral-50 p-3 sm:p-4">
              <p className="font-medium text-neutral-800 text-xs sm:text-sm">Status</p>
              <p className="mt-0.5 sm:mt-1 capitalize text-neutral-600 text-sm">{job.status}</p>
            </div>
          ...
          <div className="flex flex-wrap gap-2 sm:gap-3">
            {job.is_assigned && (
              <span className="px-2.5 sm:px-3 py-1 text-xs sm:text-sm rounded-full bg-green-100 text-green-700 font-medium">
                Assigned to you
              </span>
            )}

            {job.has_applied && !job.is_assigned && (
              <span className="px-2.5 sm:px-3 py-1 text-xs sm:text-sm rounded-full bg-blue-100 text-blue-700 font-medium">
                Application sent
              </span>
            )}
          </div>
          ...
          {/* APPLY SECTION */}
          {job.status === "open" && !job.has_applied && !job.is_assigned && (
            <div className="pt-4 sm:pt-6 border-t space-y-4 sm:space-y-5">
              {!showApplyForm ? (
                <button
                  onClick={() => setShowApplyForm(true)}
                  className="w-full sm:w-auto px-5 sm:px-6 py-2.5 sm:py-3 rounded-lg bg-black text-white font-medium
                ...
                <div className="space-y-3 sm:space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      value={price}
                      onChange={(e) => setPrice(e.target.value)}
                      className="w-full rounded-lg border px-3 sm:px-4 py-2 sm:py-2.5 h-10 sm:h-11
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      value={message}
                      onChange={(e) => setMessage(e.target.value)}
                      className="w-full rounded-lg border px-3 sm:px-4 py-2 min-h-[100px] sm:min-h-[120px]
                               focus:ring-2 focus:ring-black focus:outline-none"
                    />
                  </div>
                  ...
                  <div className="flex flex-col-reverse sm:flex-row gap-2 sm:gap-3">
                    <button
                      onClick={() => setShowApplyForm(false)}
                      className="w-full sm:w-auto px-5 sm:px-6 py-2.5 sm:py-3 rounded-lg border hover:bg-neutral-100 transition"
                    >
                      Cancel
                    </button>
                    ...
                    <button
                      onClick={submitApplication}
                      disabled={submitLoading}
                      className="w-full sm:w-auto px-5 sm:px-6 py-2.5 sm:py-3 rounded-lg bg-green-600 text-white font-medium
